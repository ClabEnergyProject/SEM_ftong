...
#    ng_fuel_cost = model_inputs['ng_fuel_cost']       
    ...
    # (1-2) For all time steps, supply must equal demand, if supply is greater than demand, curtail.
    # (3-4) Renewable energy generation (wind and solar) is prescribed by "real-time" capacity factors.
    # (5) Natural gas power generation is less than or equal to installed capacity.
    # (6) Nuclear generation constraint (must-run & fixed)
    # (7-8) Contrain the variable space for power generations.
    # (9-12) Energy storage constraints.
    # (13) Unmet demand.
    
    constraints = [
            cvx.sum_entries(dispatched_power, axis=0).T + dispatched_storage_discharge + unmet_demand == demand + dispatched_storage_charge,
            #dispatched_curtailment >= 0,
            ...
            dispatched_power[power_tech_index['nuclear'],:].T == 1 * capacity_power[power_tech_index['nuclear']],
            dispatched_power >= 0,
            capacity_power >= 0,
    ...
    # dispatched_curtailment_solution = np.asarray(dispatched_curtailment.value)         
    dispatched_curtailment_solution = 0
    
    capacity_power_solution = capacity_power.value.A1
    
#    print capacity_power.value, type(capacity_power.value), capacity_power.value.shape
#    print capacity_power_solution, type(capacity_power_solution), capacity_power_solution.shape    
    ...
    # 8 core variables, and 4 supporting informative variables.
