...
def loop_model(inputs):
    ...
    storage_charging_efficiency = inputs['storage_charging_efficiency']
    storage_charging_duration = inputs['storage_charging_duration']
    ...
    print capital_cost_wind_array.size
    print capital_cost_solar_array.size
    ...
    output_table = np.zeros([
        ...
        capital_cost_nuclear_array.size, 37 + 61])
    ...
    for ii_time in xrange(simulation_start_array.size):
        ...
        for ii_unmet in xrange(unmet_demand_cost_array.size):
            for ii_fuel_ng in xrange(ng_fuel_cost_array.size):
                for ii_cap_ng in xrange(capital_cost_ngcc_array.size):
                    for ii_cap_nuclear in xrange(capital_cost_nuclear_array.size):
                        for ii_cap_wind in xrange(capital_cost_wind_array.size):
                            for ii_cap_solar in xrange(capital_cost_solar_array.size):
                                for ii_cap_storage in xrange(capital_cost_storage_array.size):
                
                                    # -----------------------------------------------------
                                    
                                    # Pre-processing
                                    
                                    unmet_demand_cost = unmet_demand_cost_array[ii_unmet]
                                    
                                    # Fixed cost
                                    
                                    capital_cost_storage = capital_cost_storage_array[ii_cap_storage]
                                            
                                    capital_cost_power = np.array([
                                            capital_cost_ngcc_array[ii_cap_ng],
                                            capital_cost_wind_array[ii_cap_wind],
                                            capital_cost_solar_array[ii_cap_solar],                                        
                                            capital_cost_nuclear_array[ii_cap_nuclear]])
                                    ...
                                    ng_fuel_cost = ng_fuel_cost_array[ii_fuel_ng]
