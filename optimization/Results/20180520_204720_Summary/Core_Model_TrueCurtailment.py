# -*- coding: utf-8 -*-
"""
Created on Sun May 13 16:48:33 2018
...
def core_model (model_inputs):
    ...
    print "\nJust to be clear, you are calling a core model formulation that calculates curtailment in optimization\n"
...
#    ng_fuel_cost = model_inputs['ng_fuel_cost']       
    ...
    curtailment_wind = cvx.Variable(simulation_periods)
    curtailment_solar = cvx.Variable(simulation_periods)
    curtailment_nuclear = cvx.Variable(simulation_periods)
    ...
    constraints = [
            ...
            dispatched_power[power_tech_index['solar'],:].T + curtailment_solar == solar_capacity_factor * capacity_power[power_tech_index['solar']], 
            dispatched_power[power_tech_index['wind'],:].T + curtailment_wind == wind_capacity_factor * capacity_power[power_tech_index['wind']],
            dispatched_power[power_tech_index['natural_gas'],:] <= capacity_power[power_tech_index['natural_gas']], 
            dispatched_power[power_tech_index['nuclear'],:].T + curtailment_nuclear == capacity_power[power_tech_index['nuclear']],
            dispatched_power >= 0,
            capacity_power >= 0,
            curtailment_solar >= 0,
            curtailment_wind >= 0,
    ...
    fcn2min = \
        ...
        + cvx.sum_entries(curtailment_nuclear * variable_cost_power[power_tech_index['nuclear']])
        + cvx.sum_entries(curtailment_wind * variable_cost_power[power_tech_index['wind']])
        + cvx.sum_entries(curtailment_solar * variable_cost_power[power_tech_index['solar']]))
    ...
    curtailment_solar_solution = np.asarray(curtailment_solar.value)
    curtailment_wind_solution = np.asarray(curtailment_wind.value)
    curtailment_nuclear_solution = np.asarray(curtailment_nuclear.value)
